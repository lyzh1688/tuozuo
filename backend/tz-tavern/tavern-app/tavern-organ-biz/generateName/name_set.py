...
def get_name_valid(path, exist_names):
    with open('generateName/data/' + path + '.dat', encoding='utf-8') as f:
        for line in f:
            data = line.split(',')
            name = data[0][1:]
...
def get_name_txt(path, names, stroke_list):
    with open('generateName/data/' + path + '.txt', encoding='utf-8') as f:
        line_list = f.readlines()
        size = len(line_list)
        progress = 0
        for i in range(0, size):
...
def get_name_json(path, names, column, stroke_list):
    with open('generateName/data/' + path + '.json', encoding='utf-8') as f:
