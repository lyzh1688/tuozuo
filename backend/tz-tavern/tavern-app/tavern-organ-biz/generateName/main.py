...
if len(check_name) == 3:
    ...
    with open("names.txt", "w+", encoding='utf-8') as f:
        for i in get_source(int(sys.argv[1]), name_validate, get_stroke_list(sys.argv[3]+sys.argv[4], allow_general)):
        ...
        for i in names:
            if to_bool(sys.argv[6]):
                #f.write(sys.argv[3]+str(i.first_name)+sys.argv[4]+sys.argv[7] + "\t"+i.first_name[0] + "\t" + \
               #i.first_name[1] + "\t" + \
               #str(get_stroke_number(sys.argv[5])) + "\t" + \
               #str(i.stroke_number1) + "\t" + str(i.stroke_number2) + "\t" + \
               #i.source + "\n")
              print(sys.argv[3]+str(i.first_name)+sys.argv[4] + "@"+i.first_name[0] + "@" + \
              i.first_name[1] + "@" + \
              str(get_stroke_number(sys.argv[5])) + "@" + \
              str(i.stroke_number1) + "@" + str(i.stroke_number2) + "@" + \
              i.source + "\n")
               ...
               #f.write(sys.argv[3]+sys.argv[5]+str(i.first_name)+sys.argv[4] +"@"+i.first_name[0] + "@" + \
               #i.first_name[1] + "@" + \
               #str(get_stroke_number(sys.argv[5])) + "@" + \
               #str(i.stroke_number1) + "@" + str(i.stroke_number2) + "@" + \
               #i.source + "\n")
              print(sys.argv[3]+sys.argv[5]+str(i.first_name)+sys.argv[4]+sys.argv[7] +"@"+i.first_name[0] + "@" + \
              i.first_name[1] + "@" + \
              str(get_stroke_number(sys.argv[5])) + "@" + \
              str(i.stroke_number1) + "@" + str(i.stroke_number2) + "@" + \
              i.source + "\n")
